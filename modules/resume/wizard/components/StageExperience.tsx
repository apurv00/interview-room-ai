  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-sm text-slate-500">Add your roles — we&apos;ll help you write powerful bullet points</p>
      </div>

      {roles.length === 0 && (
        <div className="text-center py-8">
          <p className="text-sm text-slate-400 mb-3">No roles added yet</p>
      ...
      {roles.map((role, roleIndex) => (
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-blue-600">Role {roleIndex + 1}</span>
            {roles.length > 1 && (
              <button
                onClick={() => onRemoveRole(role.id)}
                className="text-xs text-slate-400 hover:text-[#f87171] transition-colors"
              >
                Remove
              </button>
            )}
          ...
          <div className="space-y-2">
            <label className="text-caption text-slate-500">What did you do? (one point per line)</label>
            {role.rawBullets.map((bullet, i) => (
              <div key={i} className="flex items-start gap-2">
                <span className="text-[10px] text-slate-400 mt-2.5 shrink-0">
                  {i + 1}.
                </span>
                <textarea
                  value={bullet}
                  ...
                  rows={2}
                  className="flex-1 bg-slate-100 text-sm text-slate-900 placeholder-slate-400 border border-slate-200 rounded-[6px] px-3 py-2 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-[rgba(37,99,235,0.15)] transition-all duration-[120ms] resize-none"
                />
                {role.rawBullets.length > 1 && (
                  <button
                    onClick={() => removeBullet(role.id, i, role.rawBullets)}
                    className="text-slate-400 hover:text-[#f87171] mt-2 transition-colors shrink-0"
                  >
