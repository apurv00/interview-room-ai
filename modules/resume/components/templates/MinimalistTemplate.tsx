...
export default function MinimalistTemplate({ data }: TemplateProps) {
  ...
  return (
    <div className="text-gray-900 leading-relaxed" style={{ fontSize: 'var(--r-body, 9px)' }}>
      {/* Header */}
      <div className="mb-4">
        <h1 className="font-light tracking-wide" style={{ fontSize: 'var(--r-title, 18px)' }}>{contact.fullName || 'Your Name'}</h1>
              ...
              {exp.bullets.filter(b => b.trim()).length > 0 && (
                <ul className="mt-1 space-y-1">
                  {exp.bullets.filter(b => b.trim()).map((bullet, i) => (
                    <li key={i} className="text-gray-600 pl-3 relative">
