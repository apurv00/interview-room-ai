...
export default function StartupTemplate({ data }: TemplateProps) {
  ...
  const sideProjects = data.customSections?.filter(s =>
    s.title.toLowerCase().includes('side project') || s.title.toLowerCase().includes('side-project')
  ) || []
  const interests = data.customSections?.filter(s =>
    s.title.toLowerCase().includes('interest')
  ) || []
  const otherSections = data.customSections?.filter(s =>
    !s.title.toLowerCase().includes('side project') &&
    !s.title.toLowerCase().includes('side-project') &&
    !s.title.toLowerCase().includes('interest')
  ) || []
              ...
              {proj.technologies?.length ? (
                <div className="flex flex-wrap gap-0.5 mt-0.5">
                  {proj.technologies.map((tech, i) => (
                    <span key={i} className="text-[7px] px-1 py-0.5 rounded bg-gray-100 text-gray-600">{tech}</span>
                  ))}
                </div>
              ) : null}
