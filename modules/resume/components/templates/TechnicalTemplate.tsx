...
export default function TechnicalTemplate({ data }: TemplateProps) {
  ...
  return (
    <div className="font-mono text-gray-900 leading-snug" style={{ fontSize: '10px' }}>
      {/* Header */}
      <div className="border-b-2 border-emerald-600 pb-2 mb-3">
        <h1 className="text-lg font-bold">{contact.fullName || 'Your Name'}</h1>
      ...
      {data.skills && data.skills.length > 0 && (
        <div className="mb-3">
          <h2 className="text-[9px] font-bold uppercase text-emerald-700 mb-1">Technical Skills</h2>
      ...
      {data.summary && (
        <div className="mb-3">
          <h2 className="text-[9px] font-bold uppercase text-emerald-700 mb-1">Summary</h2>
      ...
      {data.experience && data.experience.length > 0 && (
        <div className="mb-3">
          <h2 className="text-[9px] font-bold uppercase text-emerald-700 mb-1">Experience</h2>
          {data.experience.map(exp => (
            <div key={exp.id} className="mb-2">
              <div className="flex justify-between">
                <span className="font-bold text-[9px]">{exp.title} @ {exp.company}</span>
              ...
              {exp.bullets.filter(b => b.trim()).length > 0 && (
                <ul className="mt-0.5 space-y-0.5 ml-2">
                  {exp.bullets.filter(b => b.trim()).map((b, i) => (
                    <li key={i} className="text-[9px] text-gray-700 before:content-['▸'] before:mr-1 before:text-emerald-500">{b}</li>
                  ))}
                </ul>
              )}
            </div>
      ...
      {data.projects && data.projects.length > 0 && (
        <div className="mb-3">
          <h2 className="text-[9px] font-bold uppercase text-emerald-700 mb-1">Projects</h2>
      ...
      {data.education && data.education.length > 0 && (
        <div className="mb-3">
          <h2 className="text-[9px] font-bold uppercase text-emerald-700 mb-1">Education</h2>
          {data.education.map(edu => (
            <div key={edu.id} className="text-[9px] mb-1">
      ...
      {data.certifications && data.certifications.length > 0 && (
        <div className="mb-3">
          <h2 className="text-[9px] font-bold uppercase text-emerald-700 mb-1">Certifications</h2>
          {data.certifications.map((c, i) => <div key={i} className="text-[9px]">{c.name} — {c.issuer} {c.date && `(${c.date})`}</div>)}
        </div>
      )}

      {data.customSections?.map(s => (
        <div key={s.id} className="mb-3">
          <h2 className="text-[9px] font-bold uppercase text-emerald-700 mb-1">{s.title}</h2>
          <p className="text-[9px] text-gray-700 whitespace-pre-wrap">{s.content}</p>
        </div>
      ))}
    </div>
  )
