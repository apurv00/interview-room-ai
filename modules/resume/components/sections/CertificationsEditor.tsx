        ...
        <div key={idx} className="border border-slate-700 rounded-xl p-4">
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="text" value={cert.name} onChange={e => update(idx, { name: e.target.value })} placeholder="AWS Solutions Architect" className="w-full mt-1 px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
            <div>
              <label className="text-[10px] text-slate-500 uppercase tracking-wider">Issuer</label>
              <input type="text" value={cert.issuer} onChange={e => update(idx, { issuer: e.target.value })} placeholder="Amazon Web Services" className="w-full mt-1 px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="text-[10px] text-slate-500 uppercase tracking-wider">Date</label>
                <input type="text" value={cert.date || ''} onChange={e => update(idx, { date: e.target.value })} placeholder="2024" className="w-full mt-1 px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
