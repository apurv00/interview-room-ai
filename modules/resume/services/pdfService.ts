...
export function renderResumeHTML(data: ResumeData, templateId: string): string {
  const normalizedData = normalizeSkillsForPagination(data)
  const templateMarkup = renderTemplateToMarkup(normalizedData, templateId)

  const fontFamily = normalizedData.styling?.fontFamily
  const hasCustomSizes =
    normalizedData.styling?.headingSize != null || normalizedData.styling?.bodySize != null
  const sizes = hasCustomSizes
    ? getCustomFontSizes(
        normalizedData.styling?.headingSize ?? DEFAULT_HEADING_SIZE,
        normalizedData.styling?.bodySize ?? DEFAULT_BODY_SIZE,
      )
    : getFontSizes(normalizedData.styling?.fontSize)
