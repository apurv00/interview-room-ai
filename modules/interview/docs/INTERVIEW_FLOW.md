...
### 2026-08-09 · Indian voice silently became a US voice — Azure key dead in every region; Azure replaced with Sarvam (Bulbul)

- **Symptom**: candidate picks "Indian · en-IN" in the lobby; the interviewer speaks with an American voice. No visible errors, nothing in the UI.
- **Root cause**: `AZURE_SPEECH_KEY` began returning **401 from `{region}.tts.speech.microsoft.com` in EVERY region** (voices/list data-plane probe: 401 across centralindia / eastus / eastus2 / westus2 / westeurope / southeastasia ⇒ dead key or deleted/expired Speech resource, per failure mode 1 above — NOT a region mismatch). Both TTS routes gate the Indian branch on `isAzureTTSConfigured()` and 502 when the provider errors, after which the client walks its fallback chain (stream → buffered → browser `speechSynthesis`, whose top preference is "Google US English"). Net effect: a dead provider key degrades to a **silent accent swap**. Lesson: env vars being present in the deploy platform is NOT evidence the credential works — probe the data plane.
- **Fix**: Azure adapter replaced with **Sarvam AI Bulbul** (`shared/services/providers/sarvamTTS.ts`); default speaker `ishita` (Sarvam's documented top female pick for English/en-IN), tunable via `SARVAM_TTS_SPEAKER` / `SARVAM_TTS_MODEL_ID` (default `bulbul:v3`). Wire contract: POST `api.sarvam.ai/text-to-speech`, `api-subscription-key` header, **`target_language_code`** (the field name every shipping integration sends — current docs also show `language_code`; do NOT rename without a live-key test), `output_audio_codec: mp3` so the audio drops into the existing audio/mpeg MediaSource + R2-cache pipeline with zero client changes. Cache partition key `sarvam-<model>-<speaker>` — old `azure-*` R2 entries are orphaned (harmless).
- **Buffered-upstream nuance**: Sarvam's REST endpoint returns base64 JSON (no chunked stream). On a cache miss the stream route's tee flushes the Indian clip in ONE append — time-to-first-sound equals full synthesis time, not first-chunk time. The Deepgram default branch keeps true progressive streaming (the §5 TTFB budget applies to it, not to the Indian branch). R2 cache hits are unaffected. Sarvam's WebSocket streaming API cannot be proxied through these route handlers — the same constraint that kept Azure on REST.
- **Never again (loudness)**: requesting `?voice=indian` while `SARVAM_API_KEY` is unset now logs at **ERROR** level in both routes; both routes emit **`X-TTS-Provider: sarvam|deepgram`** on cache hit AND miss, so a single authenticated curl fingerprints which provider served. The adapter 502s loudly on empty audio and on WAV-despite-mp3 (codec contract drift) instead of handing MediaSource undecodable bytes.
- **Long texts**: bulbul:v3 caps a request at 2500 chars; the adapter splits at sentence/word boundaries and concatenates the MP3 frame streams (routes cap text at 5000, so at most 2 pieces).
- **Ops**: set `SARVAM_API_KEY` (from dashboard.sarvam.ai) in Coolify **production** scope (watch the preview-scope default trap) and local `.env.local`. `AZURE_SPEECH_KEY/REGION/VOICE` are retired and can be deleted from all environments.

### 2026-08-09 (addendum) · Buffered Sarvam broke interview pacing — switched to Sarvam's REST Stream endpoint

- **Symptoms (founder-reported, first Sarvam interview)**: (1) interviewer starts speaking seconds late; (2) questions advance early; (3) unprompted "Take your time…" interjections.
- **Root cause — one mechanism, three symptoms**: the first Sarvam cut used the buffered `/text-to-speech` endpoint (base64 JSON): measured **TTFB ~3.9s** for a typical question vs Deepgram's ~0.9s first chunk. The engine's conversational timers (silence nudges at useInterview's retryPrompt, turn advancement, thinking-ack scheduling) are calibrated for sub-second first-audio — ~4s of dead air per question made the loop misread the rhythm: silence nudges fired ("Take your time — whenever you're ready…"), turns advanced before/while audio played, and speech landed late. Textbook §-rule case: the latency was the disease; the pacing weirdness was symptoms.
- **Fix**: `sarvamSynthesize` now uses `POST /text-to-speech/stream` (chunked binary MP3, `output_audio_bitrate: 64k`) for texts ≤3500 chars — measured **TTFB 0.31s**, and a ~1450-char probe delivered its first byte at 0.30s while total synthesis ran 32s (true progressive streaming, not post-synthesis chunking). The raw fetch Response is returned AS-IS so the route's tee() streams live chunks; any re-wrapping would silently re-buffer (pinned by an identity assertion in the unit tests). Buffered endpoint remains only for >3500-char texts (routes cap at 5000). Content-type guard replaces the RIFF sniff on the stream path (header-only, body untouched).
- **No engine timer changes**: pacing was correct in the Deepgram era and self-heals with sub-second TTFB restored. Do not "fix" nudge timings for slow TTS — fix the TTS.
- **Note**: `enable_cached_responses` and sample-rate/pace fields deliberately omitted (provider defaults); `target_language_code` re-verified live against the stream endpoint.
- **Codex P2 hardening (same day)**: a `200 audio/mpeg` with an EMPTY body previously flowed through as success — `/api/tts` would cache zero bytes in R2, permanently silencing that text (an empty Buffer is truthy, so `if (cached)` serves it forever). The adapter now peeks the first audio chunk (zero added latency — nothing reaches the client before chunk 1 regardless), 502s on EOF-before-audio and on WAV-labeled-mp3 first bytes, and forwards the remaining chunks progressively (anti-rebuffering pinned by a gated-chunk test that deadlocks if anyone drains the stream before returning).
