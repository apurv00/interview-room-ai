...
describe('answer-candidate-question route', () => {
  ...
  it('does not treat public tech requirements as internal-tool evidence', async () => {
    const res = await POST(makeRequest({
      candidateQuestion: 'What internal tools will I use?',
      config: {
        ...baseConfig,
        jobDescription: 'Frontend role using React, TypeScript, APIs, and accessibility practices.',
      },
    }))
    const data = await res.json()
  ...
  it('does not treat generic engineering process language as hiring-process evidence', async () => {
    const res = await POST(makeRequest({
      candidateQuestion: 'What is the interview timeline?',
      config: {
        ...baseConfig,
        jobDescription: 'Improve engineering processes and offer reliable APIs for customers.',
      },
    }))
    const data = await res.json()

    expect(data.answer).toContain("I don't have the exact company-specific details here")
    expect(data.answer).toContain('next steps')
    expect(mockCompletion).not.toHaveBeenCalled()
  })
