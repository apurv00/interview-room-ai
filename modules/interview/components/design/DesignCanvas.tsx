...
function getIconForType(type: DesignComponentType) {
...
export default function DesignCanvas({ onSubmit, questionIndex, disabled = false }: DesignCanvasProps) {
  ...
  const handleAddComponent = useCallback((type: DesignComponentType) => {
    ...
    // Place in a reasonable position (stagger based on count)
    ...
    setComponents((prev) => [...prev, {
      ...
      type,
      label,
      x: Math.min(x, cw - NODE_W - 20),
      y: Math.min(y, ch - NODE_H - 20),
    }])
  ...
  const handleNodeMouseDown = useCallback((e: React.MouseEvent, nodeId: string) => {
    if (disabled) return
    if (mode === 'connect') {
      // In connect mode: click first node, then second to create connection
      if (!connectFrom) {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    const x = Math.max(0, Math.min(e.clientX - dragOffset.x, canvasRect.width - NODE_W))
    const y = Math.max(0, Math.min(e.clientY - dragOffset.y, canvasRect.height - NODE_H))
    setComponents((prev) => prev.map((c) => c.id === dragging ? { ...c, x: x - canvasRect.left, y: y - canvasRect.top } : c))
  ...
  return (
    <div className="flex flex-col h-full rounded-lg overflow-hidden border border-gray-700/50 shadow-lg">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div
          ref={canvasRef}
          className="relative flex-1 bg-gray-950 overflow-hidden cursor-crosshair"
        ...
        >
          ...
          {components.map((node) => {
            ...
            return (
              <div
                key={node.id}
                className={`absolute flex items-center gap-2 px-3 py-2 rounded-lg border cursor-grab select-none transition-colors ${
                  isConnectSource
                ...
                title={mode === 'connect' ? (isConnectSource ? 'Click another node to connect' : `Click to connect to ${node.label}`) : 'Drag to move · Double-click to delete'}
              >
          ...
          {mode === 'connect' && connectFrom && (
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-blue-600/90 text-white text-xs px-3 py-1.5 rounded-full" style={{ zIndex: 20 }}>
