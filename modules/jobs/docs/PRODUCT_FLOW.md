...
**Geo note (standing rulings #14/#24):** location accepts free text for any geography—never an India-metro dropdown—and normalizes through the same alias vocabulary used by ingestion. It is a soft ordering preference only: a match receives a visible, deterministic boost, while a miss or unknown alias leaves the complete hard-filter pool available. This does not restore the failed hard-city filter. The core loop (Fit, readiness, practice, tracker, tailor) remains fully geo-agnostic.

### A12 public discovery contract

- **Shareable navigation state:** `/jobs` derives discovery from `domain`, `q`, `location`, `remote=remote`, `experience`, `company`, `freshness`, `sort`, `cursor` and `direction`. Blank/default values are omitted. Applying any discovery change removes cursor state; browser Back/Forward, reload and a copied link restore the same public intent.
- **Filter semantics:** explicit domain, normalized search, company, freshness and remote-only are hard filters. Location is a normalized soft preference and experience (`entry | mid | senior`) is a deterministic title-token preference; neither may exclude jobs with missing or unclear data. There is intentionally no on-site-only filter because `isRemote:false` is not proof of on-site work. `Best match` is the default; `Newest` keeps strict chronological order.
- **Private personalization boundary:** resume-derived `targetRole` and `skills` never enter the URL. When present, the client sends the same public discovery state plus those private signals in the bounded, private/no-store `POST /api/jobs/feed` body. A shared URL therefore reproduces public filters, not another person's resume profile. Under `Newest`, match evidence may render but private signals do not reorder the page.
- **Stable bounded traversal:** next/previous links carry opaque, query-bound HMAC cursors. A cursor freezes the first page's creation boundary and recency clock, expires after six hours and is rejected if modified or reused with different filters. New postings do not jump into that traversal; an existing posting may still disappear if it closes. The response distinguishes the exact live hard-filter `total` from the accessible top-400 window and never presents the latter as the whole corpus.
