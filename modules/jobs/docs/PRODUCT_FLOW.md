...
`{ name, anonId?, userId?, jobPostingId?, applicationId?, sessionId?, props, ts }` — TTL 180d. `/api/events` accepts only browser-owned intent/view events: `jobs.feed_viewed`, `jobs.resume_attach_started/completed`, `jobs.target_role_confirmed`, `jobs.job_viewed`, and `jobs.prep_handoff_started`. It rejects authoritative application, outcome, worker, and identity events; those must be emitted server-side at the mutation that proves them. `jobs.prep_handoff_started` means the browser prepared a lobby handoff, while `jobs.prep_started` is reserved for verified session attendance. Browser payloads cannot set `applicationId` or `sessionId`.
