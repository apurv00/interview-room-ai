...
**Stage 0 — Stranger on public `/jobs`.** Anonymous, card-only discovery with deterministic public ranking. The discovery surface contains Search, Experience level and Sort; cards continue to show factual company, location, remote status and trustworthy posted age. A selected experience level is a hard title-evidence filter; jobs whose titles do not state a level remain available under Any. A match claim appears only when it can name actual evidence. Banner: *"Attach your resume — we'll sort these for you."* `anonId` (localStorage UUID) is minted on the first event.

**Geo note (standing ruling #14, amended by #27):** the current discovery UI does not ask for a location preference. Location remains factual card/ingestion data and the core loop (Fit, readiness, practice, tracker, tailor) remains fully geo-agnostic. The feed API temporarily accepts the retired normalized location preference for backwards compatibility, but `/jobs` removes legacy location parameters rather than applying an invisible preference.

### A12 public discovery contract

- **Shareable navigation state:** `/jobs` derives discovery from `domain`, `q`, `experience`, `sort`, `cursor` and `direction`. Blank/default values are omitted. Applying any discovery change removes cursor state; browser Back/Forward, reload and a copied link restore the same public intent. Legacy `location`, `remote`, `company` and `freshness` parameters are removed together with their cursor/direction while unrelated campaign parameters are preserved; they never remain as invisible browser filters.
- **Filter semantics:** explicit domain, normalized search and selected experience (`entry | mid | senior`) are hard public filters. Experience uses only explicit title evidence; a selected level excludes conflicting and unstated levels, while Any retains the full pool. `Best match` is the default; `Newest` keeps strict chronological order inside the same eligible pool. Separate location, work-mode, company and date-posted controls are not part of the current surface.
- **Private personalization boundary:** resume-derived `targetRole` and `skills` never enter the URL. When present, the client sends the same public discovery state plus those private signals in the bounded, private/no-store `POST /api/jobs/feed` body. Under `Best match`, one database scorer applies the recognized role domain, normalized target-title overlap and bounded resume-skill/title overlap before the top-400/page limits without making any of them a hard eligibility filter. Each resume skill counts at most once and uses the same title-evidence rule as the visible match chip. Under `Newest`, private signals never reorder results. A shared URL therefore reproduces public intent, not another person's resume profile; personalized cursor payloads are authenticated-encrypted before entering navigation URLs.
- **Stable bounded traversal:** next/previous links carry opaque, query-bound authenticated-encrypted cursors. A cursor freezes the first page's creation boundary and recency clock, expires after six hours and is rejected if modified or reused with different public intent, target-role evidence or resume-skill evidence. New postings do not jump into that traversal; an existing posting may still disappear if it closes. The response distinguishes the exact live hard-filter `total` from the accessible top-400 window and never presents the latter as the whole corpus.
- **Corpus/privacy boundary:** feed responses contain cards only—title, company, location, truthful freshness and coarse derived badges—never JD text or apply URLs. Anonymous GET and personalized POST share a Redis-backed 30-requests/minute hashed-network-identity abuse limit; missing/unavailable production Redis fails the feed read with 503 before Mongo work, and callers without a valid ingress identity share one bounded bucket. Personalized responses additionally remain private/no-store. Authenticated detail remains the only body/apply projection.

### A13 authoritative detail and action contract

- **One projection, no client invention:** authenticated detail is authoritative for posting identity, visibility, `application`/tracked state, capabilities and Apply options. Initial, background and click-time responses must be structurally valid and match the requested job ID. A malformed or wrong-job background response cannot replace the last valid projection. Missing (404), closed/expired (410), server failure and offline failure have distinct recovery copy; Retry only refetches and never mutates Save or Apply state.
- **Apply security and continuity:** static form markup retains `rel="noopener noreferrer"` as a safe default. On the interactive path the click handler synchronously opens a named blank tab, sets `popup.opener = null`, retargets the native form to that tab and removes the form `rel` only after the opener has been severed; this preserves the named POST handoff. The `/open` response adds `Referrer-Policy: no-referrer`. Popup block or unsafe setup cancels submission instead of recording a false Apply.
- **Practice authority:** the CTA is enabled only when a fresh detail projection supplies all four server-authoritative inputs: canonical JD, mapped role, signed handoff token and the active profile's validated experience. The client never substitutes a hardcoded experience. If experience is the sole missing prerequisite, `practiceBlocker:'experience-required'` sends the user to settings. Session creation re-verifies job, JD hash, role, experience and account activity, and binds the final Jobs session write to the same active profile experience inside the transaction.
- **Comparable retakes only:** exact-job retake lineage survives only when both parent and chain root match the current server-verified job, JD hash, role and experience. Any drift or lost authority routes to a new general practice without parent/comparison lineage.

**Stage 1 — Upload resume.** `/jobs/start` chooser: Upload / Build (wizard) / Import profile (authed) / one target-role question. Upload sends a PDF with a neutral filename to the stateless `/api/jobs/parse-pdf`, then sends extracted text to `/api/resume/parse` (authOptional, 10/IP/day — verified). The parsed resume remains in page memory while the reviewed role and matching skills are stored owner-scoped in **sessionStorage** for the tab; a sign-in/sign-out/account switch clears mismatched targeting. All doors converge on a review step: imported sections and parser truncation/recovery warnings are visible; feed-matching skills and **"Role you're targeting"** are editable without silently rewriting canonical resume skill groups. Absence of a parser warning is explicitly not presented as semantic accuracy. Known-loss uploads link to Builder for a deliberate re-import and full review. The one-question path gets no resume-flavored reveal copy. Personalized feed inputs travel in a bounded, private/no-store `POST /api/jobs/feed` body. The public GET accepts only the A12 discovery fields above plus first-page compatibility, and rejects resume-derived role/skill query parameters.
...
| Route | Auth | Content |
|---|---|---|
| `/jobs` | Public (deferred-auth block, `middleware.ts:143`) | URL-addressable Search, hard Experience-level filtering and Best match/Newest sorting, with authenticated-encrypted cursor navigation and a top-400 card window. Private target-domain, target-title and resume-skill/title evidence rank only inside the eligible pool before the cap/page limits through one database scorer. Anon-no-resume → Tier-A + attach banner; anon-resume → private stateless Tier-B. Flags render as demotions, never hidden. |
...
`{ name, anonId?, userId?, jobPostingId?, applicationId?, sessionId?, props, ts }` — TTL 180d. `/api/events` accepts only browser-owned intent/view events: `jobs.feed_viewed`, `jobs.resume_attach_started/completed`, `jobs.target_role_confirmed`, `jobs.job_viewed`, and `jobs.prep_handoff_started`. It rejects authoritative application, outcome, worker, and identity events; those must be emitted server-side at the mutation that proves them. `jobs.prep_handoff_started` means the browser prepared a lobby handoff, while `jobs.prep_started` is emitted only after completed, scored, job-bound practice evidence is durably attached. Browser payloads cannot set `applicationId` or `sessionId`. An authenticated capture carrying the signed anonymous cookie performs the fenced, idempotent anon→user history stitch using server-resolved identities.

The closed vocabulary also includes server/worker events: `identity_aliased`, `jobs.signup_completed{trigger}`, `jobs.job_saved`, `jobs.apply_click{tier,source=trusted-open,transitioned,evidenceVersion=1}`, `jobs.apply_confirmed{latencyMs,viaNudge}`, `jobs.broken_link{tier,hadFailover}`, `jobs.status_changed{from,to,source}`, `jobs.ghost_suggested/confirmed/auto`, `jobs.interview_scheduled{daysUntil,inferredFromPrep}`, `jobs.prep_started{applicationId,sessionId,evidenceCount}`, `jobs.prep_deferred_email`, `jobs.outcome_reported{result,round,revision,correction,followUp,fromStatus,toStatus}`, `jobs.tailor_run{jobId}`, `jobs.quickwins_viewed/clicked`, `jobs.ats_score_landed`, and `jobs.wizard_started/completed`. Undeployed names remain server-only by default until their browser ownership is explicitly added to the allowlist.
...
### 4b. The apply moment
Tier-honest button subtitles ("Opens {company}'s application form" / "Opens on {apna} — you'll need a free account" / "Via {source} — this link redirects"). Tailor is offered **beside** the button, never in front ("Tailor your resume for this job first · ~15s"). No resume → "Build a quick resume for this job" → wizard. Apply synchronously reserves a named blank tab inside the user's click gesture; if the browser blocks it, submission is cancelled and the user gets recovery guidance. On success the handler explicitly sets the blank tab's `opener` to null, retargets the native same-origin `POST /open?intent=apply` form to that name, and removes the form's static `noopener noreferrer` only after severing the opener so the named handoff remains usable; the `/open` response sends `Referrer-Policy: no-referrer`. The server transaction resolves the current canonical option, records `apply_clicked`, the exact current option generation, and a server-resolved attempt, then launches `jobs.apply_click{source=trusted-open,evidenceVersion=1}` from that canonical result as non-blocking best effort and returns 303 so the employer receives GET rather than a replayed POST. Telemetry is not awaited and cannot delay or turn a successful trusted redirect into failure. The retired `/apply-click` route remains authenticated and rate-limited only to return private/no-store `410 APPLY_CLICK_DEPRECATED`; it may read the active-account lifecycle fence but performs no posting parse, application mutation, or event write. The low-key View full posting link uses `GET /open?intent=view` and records neither an Apply status nor governance evidence; GET Apply and POST View fail closed. A signed-in direct caller can still POST `intent=apply`, and the redirect cannot prove that the external page loaded, so this is explicitly a **server-recorded resolution attempt**, not a successful visit.
...
**A12 deployment:** merge through the normal `main` workflow. Vercel and the existing Coolify webhook deploy the same commit; do not create or trigger a second staging deployment. After Coolify's health check succeeds, verify the anonymous search/filter/cursor flow at `https://staging.interviewprep.guru/jobs`. This A12 public verification needs no private staging credential and does not waive unrelated data-repair or source-control rollout gates when those systems change.
