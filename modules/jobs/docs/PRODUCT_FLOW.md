...
- **Filter semantics:** explicit domain and normalized search are hard public filters. Experience (`entry | mid | senior`) is a deterministic title-token preference and may not exclude jobs with missing or unclear data. `Best match` is the default; `Newest` keeps strict chronological order. Separate location, work-mode, company and date-posted controls are not part of the current surface.
...
| Route | Auth | Content |
|---|---|---|
| `/jobs` | Public (deferred-auth block, `middleware.ts:143`) | URL-addressable Search, Experience preference and Best match/Newest sorting, with authenticated-encrypted cursor navigation and a top-400 card window. Private target-domain, target-title and resume-skill/title evidence rank candidates before the cap/page limits through one database scorer. Anon-no-resume → Tier-A + attach banner; anon-resume → private stateless Tier-B. Flags render as demotions, never hidden. |
