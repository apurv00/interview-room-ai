...
20. **P-4 RESOLVED (founder, 2026-07-14): 35-day reversible auto-ghost.** After the 7d/21d read-time nudges go unanswered, day 35 lazily moves apply_clicked/applied rows to `ghosted` during the tracker read (ONE bulk write, `source:'system'`, status-guarded against racing fresher moves). Reversible by construction (loose machine; ghosted/rejected recoverable) and rendered as **"No response"** — the g-word never reaches users. Terminal statuses and `saved` never auto-ghost.

22. **Layered spam defense: link validation is the GENERAL layer; host-blocking is a floor, never the strategy (founder directive, 2026-07-16 — after the vacancy-spam recurrence: "you blocked railway, then we will find some other site… we should check the apply link if it is valid or not").** The hierarchy: (a) **apply-link liveness validation** (hourly machine sweep, `jobs-link-check`) is the scalable layer — works on ANY host the spammer moves to; policy ported from the probe's G4 rot checker: dead = 404/410 / DNS-nonexistent / connection-refused / 200-with-expiry-markers; bot-blocks (403/406/429/999), 5xx, and timeouts are UNVERIFIABLE and never dead (false-positive safety: a bot-blocked legit ATS must never be killed); closing requires TWO dead sweeps ≥20h apart AND every apply URL dead; crowd broken-link reports are the priority signal feeding the sweep. (b) **LLM verdict** covers content-level fraud (shadow → enforcement per #16). (c) **APPLY_DOMAIN_BLOCKLIST** stays a zero-cost instant floor for OBSERVED-abuse hosts only — never speculative, never the primary defense. (d) Manual row revocation is incident triage only — the 2026-07-15/16 lesson is that closing rows without a structural layer invites re-upload within the hour.
