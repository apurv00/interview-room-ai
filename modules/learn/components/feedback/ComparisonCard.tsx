    ...
    return (
      <div className="surface-card-bordered p-5 animate-pulse">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[1, 2, 3, 4].map(i => <div key={i} className="h-12 bg-slate-800 rounded" />)}
        </div>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-[#f0f2f5]">Compared to Previous</h3>
      ...
      {data.overallDelta !== null && (
        <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/40">
          ...
          <div className="flex-1">
            <span className="text-sm text-[#f0f2f5] font-medium">Overall Score</span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {data.dimensions.map(dim => (
          <div key={dim.dimension} className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/30">
            <ArrowIcon direction={dim.direction} />
            <div className="flex-1 min-w-0">
              <div className="text-sm text-[#d1d5db]">{dim.label}</div>
              <div className="text-xs text-[#6b7280]">{dim.current}/100</div>
            </div>
