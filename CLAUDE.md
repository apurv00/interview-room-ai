...
- **Indian interviewer voice: Azure → Sarvam (Bulbul)** — Azure Speech key went dead (401 in every region; silent fallback made every Indian-voice interview speak US English). Replaced with Sarvam `bulbul:v3`, speaker `ishita` (env `SARVAM_API_KEY` / `SARVAM_TTS_SPEAKER` / `SARVAM_TTS_MODEL_ID`); mp3 output drops into the unchanged MediaSource + R2 pipeline. Unconfigured Indian-voice requests now log at ERROR and both TTS routes emit `X-TTS-Provider` for one-curl fingerprinting; adapter 502s loudly on WAV/empty-audio contract drift. See INTERVIEW_FLOW.md §8 (2026-08-09).

- **Coding/system-design round upgrade (3-PR stack: #485 → #486 → #487)** — server-authoritative no-repeat + exemplar-seeded generation + flag-gated grounded follow-ups:
  - **#485 ServedProblem ledger**: new `ServedProblem` collection (unique `{userId, kind, problemId}`) records every served problem — AI problems upserted inside the generate-problem routes before responding (full `problemBody` persisted), static picks via fire-and-forget `POST /api/problems/served` (keepalive) at selection time. History routes return the ledger∪legacy union (capped 200 items / 64 chars each to match the generate routes' Zod). Last resort (pool exhausted + AI down) is deterministic least-recently-served instead of random repeats; `servedAt` bumps on every record so rotation rotates. Account deletion cascades the ledger; per-session delete redacts `problemBody`; GDPR export includes it. The /interview config effect waits for auth to resolve (double-run recorded phantom problems).
  - **#486 seeded generation**: both generators inject `<style_exemplars>` (static-pool exemplar + 2 QuestionBank rows via `retrieveQuestions({trackUsage:false})`) with the instruction OUTSIDE the data-boundary tags; titled avoid-list (`- Title (id)`, neutralized, inside `<already_served_problems>`, cap 30); token-Jaccard near-duplicate guard (max of title-only and title∪tags, exemplar title included) with one collision-naming retry that keeps the first candidate if the retry fails; within-band progression from the 3rd problem in a domain; AI problem ids slugged+clamped ≤64 (`toAiProblemId`) so they survive the history round-trip; design exemplar fallback map (no more URL-Shortener exemplars for ML/analyst roles).
  - **#487 grounded follow-ups (flag `grounded_followups`, default OFF)**: evaluate-code/evaluate-design generate follow-ups referencing the actual submission (design gets a second trade-off probe + `expectedComponents` gap-targeting), calibrated by the flow templates' per-band probeGuidance/neverAsk rendered as trusted SYSTEM text; client substitutes via `extractGroundedFollowUp` with hardcoded fallbacks (flag-off byte-identical). Fixes the design ask-then-never-listen bug; adds design-eval 12s timeout + 'failed'-row parity with coding (PR #456 class). maxTokens defaults bumped 1000→1400 / 1500→1800.
  - **Prod flip preconditions for `FEATURE_FLAG_GROUNDED_FOLLOWUPS=true`**: (1) one full coding + one full system-design prod self-interview (auth is prod-only); (2) verify CMS ModelConfig rows for `interview.evaluate-code`/`interview.evaluate-design` have maxTokens ≥1400/≥1800 — an active CMS slot row overrides `TASK_SLOT_DEFAULTS`, silently defeating the code-side bump.
- **Feedback scoring rebalance (G.1 → G.15, Work Items shipped)** — Full 15-item audit-driven rebalance of the feedback flow:
  - **Phase 1 — safety nets (G.1–G.6)**: baseline telemetry (`ScoreTelemetry` collection + `/cms/score-telemetry` dashboard), Zod validation at every LLM-output boundary, max-tokens truncation retry with Low-confidence surfacing, Redis-backed idempotency lock on `/api/generate-feedback` + client 202-handler poll fallback.
  - **Phase 2 — stop corrupting (G.4/G.5/G.7)**: failed evals excluded from aggregation (not averaged as placeholders), `?? null` / `== null` replaces `|| 0` / `!` silent-zero stomps across six sites, `InterviewSession.plannedQuestionCount/answeredCount/endReason/wasTruncatedByTimer` fields populated at session create + finish.
  - **Phase 3 — rebalance (G.8/G.9/G.10/G.11)**: `overall_score` blends Claude's holistic value with the deterministic formula (0.6/0.4 agreement, 0.3/0.7 safety clamp when |Δ|>20); `answer_quality.score` uses weighted mean/median/top3Mean/bottom3Mean instead of flat mean; `<3` answered → short-form guard refuses to score; `<60%` completion → score multiplier taper; `<50%` → confidence=Low; evaluate-answer prompt drops the "41-80" anchor and "every dimension" gate, adds "3 of 4 dimensions excellent / do not cluster in 55-75 / 85-92 calibration example" language.
  - **Phase 4 — transparency (G.12/G.13/G.14)**: timer-truncated answers flagged + prompt hint so Claude doesn't penalize incompleteness; head/tail transcript slice replaced with per-question-summary + full detail for the 2 weakest answers; XP/practiceStats now written server-side from `feedback.overall_score` so dashboard numbers match feedback-page numbers.
  - **Phase 5 — cleanup (G.15)**: retired six scoring-V2 feature flags after a 3-phase safe retirement (integration smoke suite → remove OFF branches one route at a time → strip flag definitions + retire legacy client XP path). `score_telemetry` flag retained permanently for ongoing observability.
  - Total: 224+ new tests across 21 scoring-related suites. Key pure helpers: `overallScore.ts` (G.8 blend), `perQAggregation.ts` (G.4/G.9), `completionAdjustment.ts` (G.10), `transcriptCompactor.ts` (G.13), `practiceStatsService.ts` (G.14). User-visible changes: overall scores now spread across the full 0-100 band per Claude's holistic judgment (not compressed to 55-75); cherry-pick 2-answer interviews refuse to score; XP matches feedback score. Operators: retired env vars `FEATURE_FLAG_SCORING_V2_*`, `FEATURE_FLAG_COMPACT_TRANSCRIPT`, `FEATURE_FLAG_XP_FROM_FEEDBACK` can be removed from deploy configs (harmless if left).
...
- **Deferred from the coding/SD round stack (#485–#487)**: (1) **Redis `SET NX` lock on problem generation** — two tabs racing selection can both trigger an LLM generation; the unique ledger index collapses the records but the spend doubles; deferred because tab-B's fallback semantics (wait vs static) were unresolved. (2) **Dedicated `interview.design-problem-gen` task slot** — `/api/design/generate-problem` still routes/bills via `interview.coding-problem-gen`; adding the slot needs a CMS ModelConfig row created at deploy time (ops-coupled). (3) **Client difficulty wiring for design generation** — the route accepts an optional `difficulty` (parity with coding's time-calibrated input) but `app/interview/page.tsx` never sends it; wire it together with an SD time-budget model (SD problems are duration-blind today).
